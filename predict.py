""" This module predicts the caption of the image as per arguments.
    ...
    usage: predict.py [-h] [--image IMAGE] [--eval] [--json_mapping JSON_MAPPING] [--cam]
                  [--verbose] [--show_image]

    optional arguments:
    -h, --help            show this help message and exit
    --verbose, -v         for detailed output of beam search
    --show_image, -s      show image with caption too

    Testing:
    For captioning group of images.

    --image IMAGE, -i IMAGE
                            folder location of images
    --eval, -e            for evaluation and finding bleu score
    --json_mapping JSON_MAPPING, -j JSON_MAPPING
                            json filepath mapping image to captions if
                            evalution==1

    WebCam:
    For using web cam

    --cam, -c             for capturing image from web cam

"""
import argparse

parser = argparse.ArgumentParser()
testing = parser.add_argument_group('Testing', 'For captioning group of images.')
testing.add_argument('--image', '-i', help="folder location of images", type=str, default='Data/Flickr_Images')
testing.add_argument('--eval', '-e', help="for evaluation and finding bleu score of given number of images",
                     action="store_true")
testing.add_argument('--json_mapping', '-j', help="json filepath mapping image to captions if evalution==1", type=str,
                     default='Data/Processed/test.json')
webcam = parser.add_argument_group('WebCam', 'For using web cam')
webcam.add_argument('--cam', '-c', help="for capturing image from web cam", action="store_true")
parser.add_argument('--verbose', '-v', help="for detailed output of beam search", action="store_true")
parser.add_argument('--show_image', '-s', help="show image with caption too", action="store_true")
parser.add_argument('--model', '-m', help="Which saved model to use?", type=str, default='model_40')
...
print(f'Using saved model: {args.model}.h5')
if args.eval:
    print(f'Finding bleu score using: {args.json_mapping}')
...
with open(f'Data/Saved Models/ImgCap_{args.model}.json') as fh:
    model_json = json.load(fh)
model = model_from_json(model_json)
model.load_weights(f'Data/Saved Models/{args.model}.h5')
...
def greedySearch(encoded_img, reference=None, model=model):
    ...
    if reference:
        ...
        bleu_score = sentence_bleu(list(map(lambda x: x.lstrip('startseq').rstrip('endseq').split(), reference)),
                                   hypothesis, smoothing_function=SmoothingFunction().method1)
        print(f'Bleu Score: {bleu_score}')
        for i in range(4):
            bleu_score = sentence_bleu(list(map(lambda x: x.lstrip('startseq').rstrip('endseq').split(), reference)),
                                       hypothesis, bleu[i], smoothing_function=SmoothingFunction().method1)
...
def main():
    if args.eval:
        try:
            ...
            with open(args.json_mapping) as fh:
            ...
            for i in range(4):
                print(
                    f'Greedy Corpus Score (nGram-{i + 1}): {corpus_bleu(references, hypothesises_greed, bleu[i], smoothing_function=SmoothingFunction().method1)}')

            print(
                f'\n\nBLEU Corpus Score: {corpus_bleu(references, hypothesises_beam, smoothing_function=SmoothingFunction().method1)}')
            for i in range(4):
                print(
                    f'BLEU Corpus Score (nGram-{i + 1}): {corpus_bleu(references, hypothesises_beam, bleu[i], smoothing_function=SmoothingFunction().method1)}')
